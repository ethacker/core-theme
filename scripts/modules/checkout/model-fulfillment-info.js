...
function ($, _, Hypr, Backbone, api, HyprLiveContext, CheckoutStep, FulfillmentContact) {
    ...
    // var FulfillmentInfo = CheckoutStep.extend({ 
        ...
        //         var self =this,
        //             contactIds = []
        //        self.getOrder().get('shippingDestinations').get('shippingDestinations').each(function(destination){
        ...
        //     }
        // });
    var FulfillmentEntry = FulfillmentContact.extend({
        getOrder: function() {
            return this.collection.parent.parent;
        } 
    ...
    var FulfillmentInfo = Backbone.Collection.extend({ 
        relations: {
        ...
        addShippingDestination: function(destination){
            var self = this;
            self.getCheckout().apModel.addShippingDestination(destination.toJSON()).then(function(data){
                self.add(new FulfillmentEntry(data));
                self.trigger('sync');
            });
        },
